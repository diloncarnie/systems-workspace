...
#include <numeric>
#include <rclcpp/rclcpp.hpp>  // ROS2 node support
...
Updater::Updater(const rclcpp::Node::SharedPtr& node)
    : mUpdaterNode(node)
{
    // Declare parameters with default values
    mUpdaterNode->declare_parameter("Camera.sigma_px", 0.002180293);
    mUpdaterNode->declare_parameter("Camera.sigma_py", 0.002186767);
    mUpdaterNode->declare_parameter("Displayer.nLandmarkPubRate", 5);
    mUpdaterNode->declare_parameter("Displayer.nLandmarkScale", 0.03);

    // Retrieve parameters
    const int nMaxTrackingLength = mUpdaterNode->get_parameter("Tracker.nMaxTrackingLength").as_int();
    mnLocalWindowSize = nMaxTrackingLength - 1;

    const double nImageNoiseSigmaX = mUpdaterNode->get_parameter("Camera.sigma_px").as_double();
    const double nImageNoiseSigmaY = mUpdaterNode->get_parameter("Camera.sigma_py").as_double();
    mnImageNoiseSigma = std::max(nImageNoiseSigmaX, nImageNoiseSigmaY);
    mnImageNoiseSigmaInv = 1.0 / mnImageNoiseSigma;

    // Assuming mSigmaInv is a 2x2 matrix (e.g., an Eigen::Matrix2d)
    mSigmaInv << std::pow(mnImageNoiseSigmaInv, 2), 0,
                 0, std::pow(mnImageNoiseSigmaInv, 2);

    // Create the Marker publisher using the node pointer.
    mFeatPub = mUpdaterNode->create_publisher<visualization_msgs::msg::Marker>(
        "/rvio2/landmarks", rclcpp::QoS(1));

    mnPubRate = mUpdaterNode->get_parameter("Displayer.nLandmarkPubRate").as_int();

    double scale = mUpdaterNode->get_parameter("Displayer.nLandmarkScale").as_double();
    scaleLandmark.x = scale;
    scaleLandmark.y = scale;
    scaleLandmark.z = scale;
                     ...
                     Eigen::MatrixXf& LocalFactor)
{
    ...
    {
        // ROS rviz settings
        visualization_msgs::msg::Marker cloud_ex;
        ...
        // ROS2: Use rclcpp::Duration to create a duration message.
        rclcpp::Duration d = rclcpp::Duration::from_seconds(1.0 / mnPubRate * mnLocalWindowSize);
        builtin_interfaces::msg::Duration lifetime_msg;
        lifetime_msg.sec = static_cast<int32_t>(d.nanoseconds() / 1000000000LL);
        lifetime_msg.nanosec = static_cast<uint32_t>(d.nanoseconds() % 1000000000LL);
        cloud_ex.lifetime = lifetime_msg;


        cloud_ex.action = visualization_msgs::msg::Marker::ADD;
        cloud_ex.type = visualization_msgs::msg::Marker::POINTS;
    ...
    {
        // ROS rviz settings
        visualization_msgs::msg::Marker cloud_is;
        ...
        // ROS2: Use rclcpp::Duration to create a duration message.
        rclcpp::Duration d = rclcpp::Duration::from_seconds(1./mnPubRate);
        builtin_interfaces::msg::Duration lifetime_msg;
        lifetime_msg.sec = static_cast<int32_t>(d.nanoseconds() / 1000000000LL);
        lifetime_msg.nanosec = static_cast<uint32_t>(d.nanoseconds() % 1000000000LL);
        cloud_is.lifetime = lifetime_msg;
        cloud_is.action = visualization_msgs::msg::Marker::ADD;
        cloud_is.type = visualization_msgs::msg::Marker::POINTS;
    ...
    {
        // ROS rviz settings
        visualization_msgs::msg::Marker cloud_po;
        ...
        lifetime_msg.sec = static_cast<int32_t>(d.nanoseconds() / 1000000000LL);
        lifetime_msg.nanosec = static_cast<uint32_t>(d.nanoseconds() % 1000000000LL);
        cloud_po.lifetime = lifetime_msg;
        cloud_po.action = visualization_msgs::msg::Marker::ADD;
        cloud_po.type = visualization_msgs::msg::Marker::POINTS;
